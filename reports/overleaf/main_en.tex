...
\begin{abstract}
\noindent
Nokia Network Services Platform (\nsp{}) is a widely deployed network service management platform in the telecommunications industry. Its core abstraction---the \emph{intent}---is a declarative JSON object that describes the desired state of a network service. Manually authoring intent JSON is challenging due to deep field paths, low fault tolerance, and high domain-knowledge requirements. This project fine-tuned the Qwen3.5-9B large language model using \lora{} (Low-Rank Adaptation) and implemented an end-to-end system that translates natural language network service requests into \nsp{} intent JSON. The implemented system covers 9 intent types (epipe, tunnel, vprn, vpls, ies, etree, cpipe, evpn-epipe, evpn-vpls) and incorporates a multi-tier validation framework grounded in Nokia's official \yang{} schemas. The final LoRA adapter was trained for 5 epochs on 2,640 training samples and evaluated on 330 held-out test samples plus 11 hand-crafted golden tests. The latest evaluation logs show 100\% JSON validity, intent-type accuracy, field recall, field precision, value accuracy, and Tier 1--4 validation pass rate on the test set. Golden tests also pass 11/11 in the final golden-only regression run. This document details the data sources, implementation, evaluation results, and remaining limitations.
...
This project builds an end-to-end Natural Language to JSON (NL-to-JSON) automation system: operators describe network service requirements in natural language (e.g., ``Create an E-Pipe service from 192.168.0.16 to 192.168.0.37''), and the system generates a concise \texttt{intent\_type + template\_name + fill\_values} object. The deterministic merge layer then expands that object into NSP RESTCONF-shaped intent JSON. Passing offline validation means the output is structurally consistent with the available \yang{} schemas and project semantic rules; live NSP server acceptance still requires future integration testing.
...
\noindent
These payloads are used by the similarity-based reference validation tier (see \S\ref{sec:tier6}). Note that 3 intent types have no canonical payloads, and existing payload field coverage varies significantly (e.g., vprn payloads range from 658 to 15 fields).
...
Chapters 4.7--4.19 of the Nokia Service Management Guide PDF provide parameter constraint descriptions and cross-field semantic rules for each service type, which were used to design and verify Tier 4 semantic validation rules.
...
The implemented system is a complete pipeline from data generation through model training, inference, merge, and validation. Figure~\ref{fig:pipeline} illustrates the system architecture.
...
The validation framework is the core quality assurance mechanism. It implements a five-tier progressive validation stack as illustrated in Figure~\ref{fig:validation-tiers}. Each tier progressively deepens the semantic level of verification.
...
\begin{tikzpicture}[
  ...
  \node[flowstep] (gen) {Field Definition Generator\\{\scriptsize field\_definitions.py}};
  \node[io, right=1.5cm of gen] (fv) {fill\_values\\{\scriptsize Field dictionary}};
  \node[flowstep, below=0.8cm of gen] (tmpl) {Template Selection\\{\scriptsize instruction\_templates.py}};
  \node[io, right=1.5cm of tmpl] (inst) {NL Instruction\\{\scriptsize User message}};
  \node[flowstep, below=0.8cm of tmpl] (valid) {Sample Validation\\{\scriptsize Tiers 1+2+4}};
  \node[flowstep, below=0.8cm of valid] (assemble) {Chat Format Assembly\\{\scriptsize (system, user, assistant)}};
...
For EVPN types (evpn-epipe, evpn-vpls), the implementation employs a \textbf{pure function generator} design. The core contract states:
...
\noindent
Deterministic derivation rules (following Nokia operator practice) are shown in Table~\ref{tab:deterministic-rules}.
...
Each intent type has natural language templates covering four linguistic styles:
...
\subsection{Data Format and Scale}
...
\midrule
Training (train.jsonl) & 2640 & SFT fine-tuning \\
Validation (val.jsonl)   & 330  & eval\_loss monitoring during training \\
Test (test.jsonl)  & 330  & Final model evaluation \\
Golden Tests (golden\_tests.jsonl) & 11 & Hand-crafted edge cases \\
\midrule
\textbf{Total} & \textbf{3311} & --- \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The 9 intent types are distributed across the generated corpus with higher representation for common and structurally complex services such as epipe, vprn, vpls, and tunnel. The golden test set is hand-crafted to cover multi-site configurations, boundary values, and easily confused types.

\subsection{Data Quality Assurance}

Every generated training sample passes the validator's Tier 1+2+4 checks before being written to JSONL, ensuring the training data itself is \yang{}-compliant and semantically correct.
...
The implemented system uses \textbf{Qwen3.5-9B} (Qwen/Qwen3.5-9B) as the base model for the following reasons:
...
\noindent
The \texttt{<think>} block is always empty and closed. At inference time, the byte sequence at the generation start position must be \textbf{byte-identical} to training. The implementation achieves this alignment by setting \texttt{enable\_thinking=False}:
...
% ============================================================
\section{Inference and Evaluation Results}
\label{sec:eval}
% ============================================================

\subsection{Inference Engine Design}

The inference module implements the following technical features:
...
The evaluation module runs full inference + validation for each test sample, reporting the metrics listed in Table~\ref{tab:eval-metrics}.
...
\noindent
Evaluation includes per-intent-type breakdowns to identify weak points. Incremental checkpoint files ensure that inference results are not lost if the process is interrupted.

\subsection{Final Metrics}

The latest full evaluation run (\texttt{output/logs/m3\_5final\_eval.log}) reports 100\% on all primary test-set metrics. The final golden-only regression run (\texttt{output/logs/golden\_only\_eval.log}) reports 11/11 golden tests passing.
...
Tier 1+2 Valid & 100.0\% & 100.0\% \\
Tier 3 Valid & 100.0\% & 100.0\% \\
Tier 4 Valid & 100.0\% & 100.0\% \\
All Tiers Valid & 100.0\% & 100.0\% \\
...
The project proceeded through five milestones, each with defined deliverables and acceptance criteria. All five milestones have been implemented in the current repository.

\begin{table}[H]
\centering
\caption{Completed project milestones}
...
% ============================================================
\section{Resolved Challenges and Mitigation Strategies}
\label{sec:challenges}
% ============================================================

\subsection{Chat Template Train-Inference Mismatch}

Qwen3.5's \texttt{<think>} tag was in inconsistent states between training and inference, causing output degradation. \textbf{Mitigation implemented}: use \texttt{enable\_thinking=False} at inference to ensure byte-level alignment.

\subsection{Information Leakage of Unlearnable Fields}

If \fillvalues{} contains random values invisible to the instruction, the model cannot learn those fields. \textbf{Mitigation implemented}: adopt the pure function generator contract for complex types like EVPN (\S\ref{sec:pure-function}), enforced by determinism tests.

\subsection{Sequence Length for Complex Topologies}

Multi-site configurations originally produced long token sequences and exposed truncation risk. \textbf{Mitigation implemented}: set max\_length to 8192 and verify sample lengths with the dry-run script before training.

\subsection{E-Tree Topology Semantics}

E-Tree services require that leaf nodes cannot communicate with each other. Naively reusing VPLS's full-mesh SDP topology violated this semantic. \textbf{Mitigation implemented}: generate only root-leaf bidirectional SDPs with a fixed 1-root + N-leaf topology.
