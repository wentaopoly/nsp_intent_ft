...
% ============================================================
% 字体与中文支持 (Overleaf 兼容配置；本地也可用 pdflatex 回退编译)
% ============================================================
\usepackage{iftex}
\ifPDFTeX
  ...
  \DeclareFontShape{C70}{rm}{m}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{b}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{bx}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{m}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{b}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{bx}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{m}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{b}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{rm}{bx}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontFamily{C70}{sf}{\hyphenchar\font\m@ne}
  \DeclareFontShape{C70}{sf}{m}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{b}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{bx}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{m}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{b}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{bx}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{m}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{b}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{sf}{bx}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontFamily{C70}{tt}{\hyphenchar\font\m@ne}
  \DeclareFontShape{C70}{tt}{m}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{b}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{bx}{n}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{m}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{b}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{bx}{sl}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{m}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{b}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \DeclareFontShape{C70}{tt}{bx}{it}{<-> CJK * gbsnu}{\CJKnormal}
  \makeatother
\else
...
\begin{abstract}
\noindent
Nokia Network Services Platform (\nsp{}) 是电信运营商广泛部署的网络服务管理平台，其核心抽象"意图"(Intent) 以声明式 JSON 描述期望的网络服务状态。手动编写意图 JSON 面临路径复杂、容错率低、知识门槛高等挑战。本项目基于 Qwen3.5-9B 大语言模型，采用 \lora{} (Low-Rank Adaptation) 参数高效微调技术，实现了一个从自然语言网络服务请求到 \nsp{} 意图 JSON 的端到端自动生成系统。当前实现覆盖 9 种意图类型（epipe, tunnel, vprn, vpls, ies, etree, cpipe, evpn-epipe, evpn-vpls），并建立了基于 Nokia 官方 \yang{} schema 的多层验证框架。最终 LoRA adapter 使用 2,640 条训练样本训练 5 个 epoch，并在 330 条测试样本和 11 条手工黄金测试上评估。最新完整测试日志显示，测试集在 JSON 有效率、意图类型准确率、字段召回、字段精度、值准确率和 Tier 1--4 验证通过率上均达到 100\%。最终 golden-only 回归运行显示 11/11 黄金测试通过。本文档总结项目的数据来源、系统实现、评估结果和剩余局限。
...
本项目构建了一个端到端的自然语言到 JSON (NL-to-JSON) 自动化系统：操作人员用自然语言描述网络服务需求（例如"创建一个从 192.168.0.16 到 192.168.0.37 的 E-Pipe 服务"），系统先生成紧凑的 \texttt{intent\_type + template\_name + fill\_values} 对象，再由确定性的 merge 层展开为 NSP RESTCONF 形态的意图 JSON。离线验证通过表示输出与当前 \yang{} schema 和项目语义规则结构一致；真实 NSP 服务端接受率仍需后续实机联调验证。

\subsection{已支持的意图类型}

系统已支持表~\ref{tab:intent-types} 所列的 9 种 \nsp{} 服务意图类型，涵盖 L2/L3 VPN、MPLS 隧道、TDM 仿真和 EVPN 等核心电信服务类别。

\begin{table}[H]
\centering
\caption{已实现的 9 种 NSP 意图类型}
...
\noindent
标准载荷已用于构建相似度参考验证层（详见 \S\ref{sec:tier6}），帮助评估模型输出的路径集合与真实部署配置的吻合程度。需要注意，3 种意图类型在 Nokia 标准包中没有对应载荷，且现有载荷的字段覆盖度参差不齐（如 vprn 的不同载荷分别包含 658 和 15 个字段）。
...
Nokia 服务管理指南 PDF 的第 4.7--4.19 章节提供了各服务类型的参数约束说明和跨字段语义规则描述，已用于设计和验证 Tier 4 语义校验规则的正确性。
...
当前系统已经实现一条完整的「数据生成 $\to$ 模型训练 $\to$ 推理 $\to$ merge $\to$ 验证」流水线。图~\ref{fig:pipeline} 展示了系统的整体架构。
...
Schema 解析器是整个系统的数据基础设施。实现中使用 \texttt{pyang} 库解析 Nokia 的 \yang{} 模块，构建统一的 \texttt{SchemaIndex} 索引。核心设计包括：
...
@dataclass
class LeafMeta:
    path: str               # normalized YANG path
    base_type: str          # base type (int32/string/boolean/...)
    range_expr: str | None  # numeric range
    length_expr: str | None # string length constraint
    pattern_list: list[str] # regex patterns
    enum_values: list[str]  # enum value set
    mandatory: bool         # mandatory leaf
    is_leaf_list: bool      # leaf-list marker
...
验证框架是系统质量保障的核心。当前实现了五层递进式校验栈，如图~\ref{fig:validation-tiers} 所示，逐步提升校验的语义深度。
...
校验 \yang{} schema 无法表达但业务上必须满足的跨字段约束。表~\ref{tab:semantic-rules} 列出每种意图类型已实现的语义规则。
...
\begin{tikzpicture}[
  ...
  \node[flowstep] (gen) {字段定义生成器\\{\scriptsize field\_definitions.py}};
  \node[io, right=1.5cm of gen] (fv) {fill\_values\\{\scriptsize 字段字典}};
  \node[flowstep, below=0.8cm of gen] (tmpl) {指令模板选择\\{\scriptsize instruction\_templates.py}};
  \node[io, right=1.5cm of tmpl] (inst) {自然语言指令\\{\scriptsize user message}};
  \node[flowstep, below=0.8cm of tmpl] (valid) {样本验证\\{\scriptsize Tier 1+2+4}};
  \node[flowstep, below=0.8cm of valid] (assemble) {组装 chat 格式\\{\scriptsize (system, user, assistant)}};
...
对于 EVPN 类型（evpn-epipe、evpn-vpls），实现中采用\textbf{纯函数生成器}设计。其核心契约是：
...
# Step 1: sample all random values as instruction-visible args
args = _roll_evpn_epipe_args()

# Step 2: pure generation with no internal random calls
fill_values = generate_evpn_epipe_values(**args)

# Step 3: drive the instruction template with the same args
instruction = template.format(**args)
\end{lstlisting}

\noindent
确定性推导规则（参考 Nokia 运营实践）如表~\ref{tab:deterministic-rules} 所示。
...
验证集 (val.jsonl)   & 330  & 训练中的 eval\_loss 监控 \\
测试集 (test.jsonl)  & 330  & 最终模型评估 \\
黄金测试 (golden\_tests.jsonl) & 11 & 手工编写的边界用例 \\
\midrule
\textbf{总计} & \textbf{3311} & --- \\
\bottomrule
\end{tabular}
...
\noindent
9 种意图类型均覆盖在数据集中；常用或结构复杂的类型（如 epipe、vprn、vpls、tunnel）样本占比更高。黄金测试集为手工编写，覆盖多站点配置、边界值、易混淆类型等难例。

\subsection{数据质量保障}

每个生成的训练样本在写入 JSONL 前都通过验证器的 Tier 1+2+4 校验，确保训练数据本身的质量。这意味着模型学习到的每一个样本都是 \yang{} schema 合规且语义正确的。
...
\noindent
即 \texttt{<think>} 块始终为空且已关闭。推理时，必须确保生成起始位置的字节序列与训练时\textbf{完全一致}。当前实现通过设置 \texttt{enable\_thinking=False} 实现这一对齐：
...
评估模块对每个测试样本运行完整推理+验证流程，报告如表~\ref{tab:eval-metrics} 所列指标。
...
\noindent
评估按意图类型细分报告，以识别特定类型的薄弱环节。评估过程使用增量检查点文件 (checkpoint JSONL)，确保即使中途中断也不丢失已完成的推理结果。

\subsection{最终指标}

最新完整评估运行（\texttt{output/logs/m3\_5final\_eval.log}）显示，测试集全部主指标均为 100\%。最终 golden-only 回归运行（\texttt{output/logs/golden\_only\_eval.log}）显示 11/11 个黄金测试通过。
...
\begin{tabular}{lcc}
\toprule
\textbf{指标} & \textbf{测试集 (330)} & \textbf{黄金测试 (11)} \\
\midrule
...
Tier 6 Canonical Recognition & 96.6\% & 信息性指标 \\
\bottomrule
\end{tabular}
\end{table}
...
项目按五个里程碑推进，每个里程碑有明确的交付物和验收标准。当前仓库已经完成全部五个里程碑的实现和验证。
...
Qwen3.5 的 \texttt{<think>} 标记在训练和推理时的状态不一致曾导致模型输出退化。已实施的应对策略：在推理时使用 \texttt{enable\_thinking=False} 确保字节级对齐。

\subsection{不可学习字段的信息泄漏}

如果 \fillvalues{} 中存在指令不可见的随机值，模型无法学习这些字段。已实施的应对策略：对 EVPN 等复杂类型采用纯函数生成器契约（\S\ref{sec:pure-function}），并实施确定性测试加以保障。

\subsection{复杂拓扑的序列长度}

多站点配置曾带来长序列和截断风险。已实施的应对策略：将 max\_length 设置为 8192，并通过 dry-run 脚本在训练前验证样本长度。

\subsection{E-Tree 拓扑语义}

E-Tree 服务要求叶节点之间不可通信，直接复用 VPLS 的全网状 SDP 拓扑会违反此语义。已实施的应对策略：仅生成 root-leaf 双向 SDP，固定 1 个 root + N 个 leaf 的拓扑。

\subsection{标准载荷覆盖不完整}

etree、cpipe、tunnel 在 Nokia 标准包中没有标准载荷，Tier 6 对这些类型无法提供参考。当前处理方式：将 Tier 6 定义为信息性指标（不阻断），并在 Tier 1--4 中确保充分的结构和语义校验。
